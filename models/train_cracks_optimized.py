...
# 只训练裂缝数据集
DATASET = str(PROJECT_ROOT / 'datas' / 'cracks' / 'data.yaml')
...
def train_all_models():
    ...
    print(f"\n{'='*60}")
    print(f"开始训练裂缝数据集 - 消融实验（混合方案）")
    print(f"{'='*60}\n")
    
    for model_name, model_config in MODELS.items():
        print(f"\n训练模型: {model_name}")
        exp_name = f"cracks_{model_name.replace('+', '_').replace('-', '_')}"
...
def plot_loss_comparison(training_data, save_dir='runs/comparison_cracks'):
    """绘制损失对比图"""
    ...
    plt.title('裂缝数据集 - 训练损失对比', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_dir / 'cracks_train_loss.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✓ 保存训练损失图: cracks_train_loss.png")
    ...
    plt.title('裂缝数据集 - 验证损失对比', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(save_dir / 'cracks_val_loss.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✓ 保存验证损失图: cracks_val_loss.png")


def plot_pr_curves(results, save_dir='runs/comparison_cracks'):
    """绘制P-R曲线对比图"""
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    plt.title('裂缝数据集 - P-R曲线对比', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10, loc='best')
    plt.grid(True, alpha=0.3)
    plt.xlim([0, 1])
    plt.ylim([0, 1])
    plt.tight_layout()
    plt.savefig(save_dir / 'cracks_pr_curve.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✓ 保存P-R曲线图: cracks_pr_curve.png")


def generate_summary_report(training_data, results, save_dir='runs/comparison_cracks'):
    """生成训练结果汇总报告"""
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    report_lines.append("裂缝数据集训练结果汇总")
    report_lines.append("=" * 80)
    report_lines.append("")
    report_lines.append("数据集: CRACKS")
    report_lines.append("-" * 80)
    ...
    # 保存到文件
    report_file = save_dir / 'cracks_training_summary.txt'
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60)
    print("裂缝数据集训练完成！")
    print("结果保存在: runs/comparison_cracks/")
    print("="*60)
